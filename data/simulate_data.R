...
my_data <- data.frame(part_ID = paste0("PID_", 1:N),
                      complexity_level = sample(x = c("low", "high"), 
                                                size = N, 
                                                replace = TRUE),
                      message_framing = sample(x = c("prevention", "promotion"), 
                                               size = N, 
                                               replace = TRUE),
...
my_data <- my_data %>%
  mutate(product_satisfaction = 2 + 
                                main_cplx * is_high_cplx + 
                                main_frame * is_promotion +
                                main_cplx_frame * is_high_cplx * is_promotion + 
                                unobs_noise)
...
write_csv(my_data,
          file = here::here("data", "simulated_data.csv"))
